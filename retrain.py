...
def retrain(args, data, edges_to_forget, device, forget_all=False):
    ...
    for edge in tqdm(edges_to_forget, desc='  retraining'):
        ...
        data_['edges'] = edges_
        retrain_model = train_gcn(args, data_, eval=False, device=device, verbose=False)
        torch.save(retrain_model.state_dict(), os.path.join('./checkpoint', f'gcn_{args.data}_{v1}_{v2}_best.pt'))
        edges_ = [(v1, v2) for v1, v2 in data['edges']]
...
def loss_difference(args, data, edges_to_forget, device):
    ...
    for edge in tqdm(edges_to_forget, desc='  loss diff'):
        edges_.remove(edge)
